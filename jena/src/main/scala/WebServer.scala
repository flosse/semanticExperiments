...
class AddStatementHandler( model:Model, statementType:String ) extends HttpHandler {
  ...
  def handle( exchange: HttpExchange ) {
    ...
    try{
      ...
      val s = model.createResource( params("subject") )
      val p = model.createProperty( params("predicate") )
      val o = statementType match {
        case "literal"  =>  model.createLiteral( params("object") )
        case "resource" =>  model.createResource( params("object") ) 
...
class WebServer( model:Model, queryModule:QueryModule, port:Int ){
  ...
  server.createContext("/add/ResourceStatement", new AddStatementHandler( model, "resource" ) )
  server.createContext("/add/LiteralStatement", new AddStatementHandler( model, "literal" ) )
